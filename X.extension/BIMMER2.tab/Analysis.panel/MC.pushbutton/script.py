...
def strmap2dict(str1):
    return pipe(str1.split(":"), filter(lambda x: len(x) > 0), map(lambda x: x.rstrip()), map(lambda x: x.split(" ")), dict)
...
def flow_params(x):
    p = strmap2dict(get_parameter_value(get_family_type(x), "mc-PW"))
    gpm = float(p['flowrate_gpm'])
    gph = float(p['average_gph'])
    return {'MC-flow': gpm, 'MC-gph': gph}
